...
class Confusables:
    ...
    def _full_confusable_graphemes(self) -> Dict[str, Tuple[str, List[str]]]:
        with open(
            get_resource_path(self.config.inspector.confusables), "r", encoding="utf-8"
        ) as f:
            old_confusables = json.load(f)

        with open(
            get_resource_path(self.config.inspector.grapheme_confusables),
            "r",
            encoding="utf-8",
        ) as f:
    ...
    def _simple_confusable_graphemes(self) -> Dict[str, Tuple[str, List[str]]]:
        ...
        return {
            g: (
                canon if canon is not None and is_simple_confusable(canon) else None,
    ...
    def is_confusable_grapheme(self, grapheme: str) -> bool:
        ...
        if grapheme in self.confusable_graphemes:
            if (
                self.confusable_graphemes[grapheme][0] == grapheme
                and not self.confusable_graphemes[grapheme][1]
            ):
                return False
            else:
                return True
    ...
    def is_confusable(self, string: str) -> bool:
        return self.is_confusable_grapheme(
            string
        )  # or (len(string) > 1 and self.is_confusable(string[0]))

    def get_confusables(self, string: str) -> List[str]:
        confusables = self.get_confusables_grapheme(string)
        return (
            confusables
            if confusables or len(string) == 1
            else self.get_confusables(string[0])
        )
    ...
    def get_canonical(self, string: str) -> Optional[str]:
        ...
        return (
            canonical
            if canonical or len(string) == 1
            else self.get_canonical(string[0])
        )
