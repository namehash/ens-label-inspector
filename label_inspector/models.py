...
class InspectorCharResult(BaseModel):
    ...
    script: str = Field(
        description="Script name (writing system) of the character.\n"
    ...
    name: str = Field(
        description="Name assigned to the character.\n"
        "* for unknown characters it is `Unknown character in <script> script`"
    )
    codepoint: str = Field(
        description="Codepoint of the character as hex integer with 0x prefix."
    )
    link: str = Field(
        description="Link to an external page with information about the character."
    )
    type: str = Field(
...
class InspectorGraphemeResult(BaseModel):
    ...
    chars: List[InspectorCharResult] = Field(
        description="List of characters. May be shorter than `value` (grapheme string) if `truncate_chars` applies"
    )
    name: str = Field(
        description="Name of the grapheme.\n"
        "* is `Combined Character` for multi-character graphemes without a unique name"
    )
    codepoint: Optional[str] = Field(
        description="Codepoint of the single-character grapheme as hex with 0x prefix.\n"
        "* `null` for multi-character graphemes"
    )
    link: Optional[str] = Field(
        description="Link to an external page with information about the single-character grapheme.\n"
        "* `null` for multi-character graphemes"
    )
    script: str = Field(
...
class InspectorConfusableMultiGraphemeResult(BaseModel):
    ...
    chars: List[InspectorCharResult] = Field(
        description="List of characters in the confusable."
    )


InspectorConfusableResult = Union[
    InspectorConfusableGraphemeResult, InspectorConfusableMultiGraphemeResult
]


class InspectorGraphemeWithConfusablesResult(InspectorGraphemeResult):
    confusables_canonical: Optional[InspectorConfusableResult] = Field(
        description="Canonical form of confusable grapheme.\n"
    ...
    confusables_other: List[InspectorConfusableResult] = Field(
        description="List of confusable forms without the canonical.\n"
...
class InspectorResultBase(BaseModel):
    ...
    status: str = Field(
        description="Status of the input label.\n"
    ...
    all_type: Optional[str] = Field(
        description="Type of all graphemes if all graphemes have the same type, otherwise `null`."
    )

    any_types: List[str] = Field(
        description="List of all unique grapheme types that are present in the input label."
    )
    ...
    any_scripts: List[str] = Field(
        description="List of unique script names of all graphemes."
    )

    confusable_count: int = Field(
        description="Number of graphemes that are confusable."
    )
...
class InspectorResultNormalized(InspectorResultBase):
    beautiful_label: str = Field(
        description="ENSIP-15 beautified version of the input label."
    )
...
class InspectorResultUnnormalized(InspectorResultBase):
    ...
    normalization_error_message: str = Field(
        description="Reason why the input label is not normalized."
    )

    normalization_error_details: Optional[str] = Field(
        description="Details of the normalization error."
    )

    normalization_error_code: str = Field(
        description="Error code of the normalization error.\n" "\n".join(
            f"* `{e.code}` - {e.general_info}" for e in DisallowedSequenceType
        )
        + "\n"
        + "\n".join(f"* `{e.code}` - {e.general_info}" for e in CurableSequenceType)
        + "\n"
        + "\n".join(
            f"* `{e.code}` - {e.general_info}" for e in NormalizableSequenceType
        )
    )
...
class InspectorBatchResult(BaseModel):
    results: List[InspectorResult] = Field(
        description="List of results for each input label."
    )
