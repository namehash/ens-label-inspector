...
def analyse_label():
    with initialize_inspector_config("prod_config") as config:
        ...
        return lambda label, *args, **kwargs: inspector.analyse_label(
            label, *args, **kwargs
        ).model_dump()
...
def test_inspector_numerics():
    with initialize_inspector_config("prod_config") as config:
        ...
        with open(
            os.path.join(TESTS_DATA_PATH, "unicode_numerics.txt"), "r", encoding="utf-8"
        ) as f:
            for line in f:
                line = line.strip()
...
def test_inspector_aggregation_works_before_truncation(analyse_label):
    ...
    assert [c["value"] for g in result["graphemes"] for c in g["chars"]] == [
        "a",
        "b",
        "c",
    ]
    assert result["all_type"] is None
...
def test_inspector_label_of_unknown_char(analyse_label):
    ...
    result = analyse_label(label)
    assert (
        result["graphemes"][0]["chars"][0]["label"]
        == "Unknown character in Unknown script"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            None,
        ),  # "˪" is confusable, but has no canonical version
        ("-ś", True, "-s", "-s"),  # canonical_label length is less than 3
        # TODO find new example
        pytest.param(
            "𐌂𐌂𐌂", True, "ccc", "ccc", marks=pytest.mark.xfail
        ),  # canonical_label gets normalized,
        ...
        (
            ...
            "🧟" * 3,
            "🧟\ufe0f" * 3,
        ),  # canonical version is a simple zombie
    ],
)
...
def test_inspector_all_script2(analyse_label, label, script):
    result = analyse_label(label)
    assert result["all_script"] == script
...
@pytest.mark.parametrize(
    "label,g_link,c_links",
    [
        ("a", "https://unicodeplus.com/U+0061", ["https://unicodeplus.com/U+0061"]),
        (
            "-\u0610",
            "https://unicode.link/inspect/utf8:2d.d8.90",
            ["https://unicodeplus.com/U+002D", "https://unicodeplus.com/U+0610"],
        ),
        ("🧌", "http://📙.la/🧌", ["http://📙.la/🧌"]),
        (
            "👩🏿‍🦲",
            "http://📙.la/👩🏿‍🦲",
            ["http://📙.la/👩", "http://📙.la/🏿", "http://📙.la/‍", "http://📙.la/🦲"],
        ),
    ],
)
...
def test_inspector_punycode(analyse_label):
    ...
    assert resp["dns_hostname_support"] is False
    ...
    assert resp["punycode_encoding"] is None


def test_inspector_font_support(analyse_label):
    resp = analyse_label("a✊🏾")
    assert resp["font_support_all_os"] is True
    assert resp["graphemes"][0]["font_support_all_os"] is True
    assert resp["graphemes"][1]["font_support_all_os"] is True

    resp = analyse_label("✊🏾" + chr(5922) + "👊🏿")
    assert resp["font_support_all_os"] is False
    assert resp["graphemes"][0]["font_support_all_os"] is True
    assert resp["graphemes"][1]["font_support_all_os"] is False
    assert resp["graphemes"][2]["font_support_all_os"] is False
