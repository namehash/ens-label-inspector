...
def test_prod_inspector_long_post(prod_test_client):
    ...
    response = client.post(
        "/",
        json={
            "label": "miinibaashkiminasiganibiitoosijiganibadagwiingweshiganibakwezhigan"
        },
    )
...
def test_inspector_limit_confusables(prod_test_client):
    label = "ącat"
    response = prod_test_client.post(
        "/", json={"label": label, "truncate_confusables": 1}
    )
    assert response.status_code == 200
    json = response.json()
    print(json)
...
def test_inspector_disable_chars_output(prod_test_client):
    label = "cat"
    response = prod_test_client.post(
        "/", json={"label": label, "truncate_graphemes": 0}
    )
    assert response.status_code == 200
    json = response.json()
    print(json)
...
def test_prod_inspector_confusable(prod_test_client):
    client = prod_test_client
    response = client.post("/", json={"label": "tést"})
...
def test_inspector_truncate_graphemes(prod_test_client):
    label = "-\u0610-\u0610"
    response = prod_test_client.post(
        "/", json={"label": label, "truncate_graphemes": 1}
    )
    assert response.status_code == 200
    json = response.json()
...
def test_inspector_truncate_graphemes_and_chars(prod_test_client):
    label = "-\u0610-\u0610"
    response = prod_test_client.post(
        "/", json={"label": label, "truncate_graphemes": 1, "truncate_chars": 1}
    )
    assert response.status_code == 200
    json = response.json()
...
@pytest.mark.parametrize(
    "label,is_normalized",
    [("goodname", True), ("unnorma\u0328", False), ("bad_name", False)],
)
