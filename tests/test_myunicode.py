...
def test_name_throws_on_str(chr):
    with pytest.raises(
        TypeError, match=r"name\(\) argument 1 must be a unicode character, not str"
    ):
        myunicode.name(chr)
    with pytest.raises(
        TypeError, match=r"name\(\) argument 1 must be a unicode character, not str"
...
def test_category_throws_on_str(chr):
    with pytest.raises(
        TypeError, match=r"category\(\) argument must be a unicode character, not str"
    ):
...
def test_combining_throws_on_str(chr):
    with pytest.raises(
        TypeError, match=r"combining\(\) argument must be a unicode character, not str"
    ):
...
def test_block_of_throws_on_str(chr):
    with pytest.raises(
        Exception, match=r"block_of\(\) argument must be a unicode character, not str"
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "\U0001F469\U0001F3FB\U0000200D\U0001F91D\U0000200D\U0000200D\U0001F469\U0001F3FC",
            False,
        ),  # 2 ZWJs
    ],
)
...
def test_all_characters():
    ...
    for char in map(chr, range(0x10FFFF + 1)):
        ...
        assert myunicode.combining(char) == unicodedata.combining(char), hex(ord(char))[
            2:
        ]
        assert myunicode.category(char) == unicodedata.category(char), hex(ord(char))[
            2:
        ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "\U0001F469\U0001F3FF\U0000200D\U0001F9B2\ufe0f",
            "WOMAN: DARK SKIN TONE, BALD WITH VARIATIONAL SELECTOR(S)",
        ),
...
def test_grapheme_iter_no_hangul():
    ...
    for line in lines:
        if any(
            block.find("Hangul") != -1 and block.find("Jamo") != -1
            for c in line
            for block in [myunicode.block_of(c) or ""]
        ):
            continue
