...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-simple",
        action="store_true",
        help=f"Skip simple labels {SPECIAL_CHAR_REGEX.pattern}",
    )
    parser.add_argument(
        "-t", "--timeout", type=float, default=0, help="Timeout [s] for each request"
    )
    parser.add_argument(
        "-l",
        "--long-length",
    ...
    def test_label(label):
        if (
            len(label) < long_length
            and enable_filter
            and SPECIAL_CHAR_REGEX.search(label) is None
        ):
            return
        ...
        assert duration < timeout, "Time limit exceeded"
