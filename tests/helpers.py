...
def check_inspector_base_response(
    label, resp, truncate_confusables=None, truncate_graphemes=None, truncate_chars=None
):
    ...
    for grapheme in resp["graphemes"]:
        ...
        assert isinstance(grapheme["value"], str)
        assert isinstance(grapheme["chars"], list)
        assert isinstance(grapheme["name"], str)
        assert isinstance(grapheme["script"], str)
        assert grapheme["confusables_canonical"] is None or isinstance(
            grapheme["confusables_canonical"], dict
        )
        if grapheme["confusables_canonical"] is not None:
            if (
                len(
                    myunicode.grapheme.split(grapheme["confusables_canonical"]["value"])
                )
                != 1
            ):
    ...
    name_chars = (
        ...
        else (c for g in myunicode.grapheme.split(label) for c in g[:truncate_chars])
    )
    for char, name_char in zip(all_chars, name_chars):
        ...
        assert isinstance(char["script"], str)
        ...
        assert isinstance(char["type"], str)


def check_inspector_normalized_response(
    label, resp, truncate_confusables=None, truncate_graphemes=None, truncate_chars=None
):
    assert sorted(resp.keys()) == sorted(
        BASE_RESPONSE_FIELDS + NORMALIZED_RESPONSE_FIELDS
    )
...
def check_inspector_unnormalized_response(
    label, resp, truncate_confusables=None, truncate_graphemes=None, truncate_chars=None
):
    assert sorted(resp.keys()) == sorted(
        BASE_RESPONSE_FIELDS + UNNORMALIZED_RESPONSE_FIELDS
    )
    ...
    if isinstance(error, CurableSequence):
        ...
        assert (
            resp["label"][
                resp["disallowed_sequence_start"] : resp["disallowed_sequence_start"]
                + len(error.sequence)
            ]
            == error.sequence
        )
        assert (
            "".join(c["value"] for c in resp["disallowed_sequence"]) == error.sequence
        )
        assert (
            "".join(c["value"] for c in resp["suggested_replacement"])
            == error.suggested
        )
    else:
...
def check_inspector_response(
    ...
    if is_ens_normalized(label):
        check_inspector_normalized_response(
            label,
            resp,
            truncate_confusables=truncate_confusables,
            truncate_graphemes=truncate_graphemes,
        ...
        check_inspector_unnormalized_response(
            label,
            resp,
            truncate_confusables=truncate_confusables,
            truncate_graphemes=truncate_graphemes,
...
def generate_example_names(
    count, input_filename=os.path.join(TESTS_DATA_PATH, "primary.csv")
):
...
def load_new_unicode_chars(unicode_version: str) -> List[Tuple[str, str]]:
    ...
    """
    with open(
        os.path.join(TESTS_DATA_PATH, f"unicode{unicode_version}.txt"), encoding="utf-8"
    ) as f:
        chars = []
        for line in f:
            line = line.strip()
